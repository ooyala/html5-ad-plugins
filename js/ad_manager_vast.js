...
OO.Ads.manager(() => {
  ...
  const Vast = function () {
    ...
    const _handleTrackingUrls = (amcAd, trackingEventNames) => {
      if (!_isVpaidAd(amcAd)) {
        ...
        if (amcAd) {
          ...
          OO.log(
            `VAST: Tried to ping URLs: [${trackingEventNames}] but ad object passed in was: ${amcAd}`,
          );
        }
     ...
     * @param {object} currentAdObj In order to not lose reference to the overlay object that is currently being shown, it
     * is stored in this object
     */
    this.hideOverlay = (currentAdObj) => {
      this.lastOverlayAd = currentAdObj;
    };
    ...
    this.showOverlay = () => {
      if (this.amc.ended && this.lastOverlayAd) {
        this.cancelAd(this.lastOverlayAd);
      }
    };
    ...
    const handleAds = (ads, adLoaded, fallbackAd) => {
      ...
      if (fallbackAd) {
        ...
        } else if (fallbackAd.type === AD_TYPE.WRAPPER) {
          _handleWrapperAd(fallbackAd, adLoaded);
          handlingWrapperAd = true;
          handled = true;
        }
      }
