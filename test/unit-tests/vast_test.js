...
describe('ad_manager_vast', function() {
  ...
  it('Vast 3.0, VMAP: Should parse AdBreak with repeatAfter attribute properly', function() {
    ...
    expect(vastAd.ad.data.linear.mediaFiles[0].maintainAspectRatio).to.be(undefined);
    expect(vastAd.ad.data.linear.mediaFiles[0].url).to.be("1.mp4");
    expect(vastAd.ad.repeatAfter).to.be(undefined);
    expect(vastAd.ad.firstRepeatAdPlayed).to.be(false);
  });

  it('Vast 3.0, VMAP: Should play repeat ads properly without seek', function() {
    var duration = 100;
    var adQueue = [];
    amc.forceAdToPlay = function(adManager, ad, adType, streams) {
      var adData = {
    ...
    expect(amc.timeline[0].ad.firstRepeatAdPlayed).to.be(false);
    expect(amc.timeline[1].ad.firstRepeatAdPlayed).to.be(false);

    // play first ads
    vastAdManager.playAd(amc.timeline[0]);
    vastAdManager.playAd(amc.timeline[1]);
    expect(amc.timeline[0].ad.firstRepeatAdPlayed).to.be(true);
    expect(amc.timeline[1].ad.firstRepeatAdPlayed).to.be(true);

    // repeat-ad-1 set to play every 5 seconds
    // repeat-ad-2 set to play every 10 seconds
    expect(vastAdManager.getRepeatAds().length).to.be(2);

    // repeat-ad-1 only
    vastAdManager.onPlayheadTimeChanged(OO.EVENTS.PLAYHEAD_TIME_CHANGED, 5, duration);
    expect(adQueue.length).to.be(1);
    adQueue = [];

    // repeat-ad-1 and repeat-ad-2 overlap
    vastAdManager.onPlayheadTimeChanged(OO.EVENTS.PLAYHEAD_TIME_CHANGED, 10, duration);
    expect(adQueue.length).to.be(2);
    adQueue = [];

    // original adbreak (mid-roll-1) at 15 seconds, so no repeat ads should be added to the adQueue
    adQueue = [amc.timeline[2]];
    vastAdManager.onPlayheadTimeChanged(OO.EVENTS.PLAYHEAD_TIME_CHANGED, 15, duration);
    expect(adQueue.length).to.be(1);
    adQueue = [];

    // repeat-ad-1 and repeat-ad-2 overlap
    vastAdManager.onPlayheadTimeChanged(OO.EVENTS.PLAYHEAD_TIME_CHANGED, 20, duration);
    expect(adQueue.length).to.be(2);
    adQueue = [];

    // repeat-ad-1 only
    vastAdManager.onPlayheadTimeChanged(OO.EVENTS.PLAYHEAD_TIME_CHANGED, 25, duration);
    expect(adQueue.length).to.be(1);
    adQueue = [];

    // repeat-ad-1 and repeat-ad-2 overlap
    vastAdManager.onPlayheadTimeChanged(OO.EVENTS.PLAYHEAD_TIME_CHANGED, 30, duration);
    expect(adQueue.length).to.be(2);
    adQueue = [];
  ...
  it('Vast 3.0, VMAP: Repeat ads should not play when scrubbed past midroll', function() {
    var duration = 100;
    var adQueue = [];
    amc.forceAdToPlay = function(adManager, ad, adType, streams) {
      var adData = {
    ...
    amc.getPositionOfCurrentAd = function() {
      if (adQueue.length > 0) {
        return adQueue[0].ad.positionSeconds;
      }
      return null;
    };
    ...
    // original adbreak (mid-roll-1) at 15 seconds, so no repeat ads should be added to the adQueue
    adQueue = [amc.timeline[2]];
    vastAdManager.onSeeked(OO.EVENTS.SEEKED, 16);
    expect(adQueue.length).to.be(1);
    adQueue = [];
  ...
  it('Vast 3.0, VMAP: Repeat ads should be played when scrubbed past their intervals', function() {
    var duration = 100;
    var adQueue = [];
    amc.forceAdToPlay = function(adManager, ad, adType, streams) {
      var adData = {
    ...
    // original adbreak (mid-roll-1) at 15 seconds, so no repeat ads should be added to the adQueue
    vastAdManager.onSeeked(OO.EVENTS.SEEKED, 11);
    expect(adQueue.length).to.be(2);
    adQueue = [];
